...
def katakana_kantei(user_data):
    kusei_nen = int(user_data['kusei_nen'])
    kusei_tuki = int(user_data['kusei_tuki'])
    kusei_hi = int(user_data['kusei_hi'])

    #馬名を取得
    for i in range(18):
        if user_data['uma' + str(i +1)] != '':
            uma.append(user_data['uma' + str(i +1)])
            uma_l = i +1
        else:
            break
    ...
    #騎手名を取得
    for i in range(18): 
        if user_data['kisyu' + str(i +1)] != '':
            kisyu.append(user_data['kisyu' + str(i +1)])
            kisyu_l = i +1
        else:
            break
    ...
    return kantei_uma_total,kantei_kisyu_total
